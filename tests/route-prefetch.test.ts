...
describe('route prefetch network policy', () => {
  test('allows fast and unknown network conditions', () => {
    expect(shouldPrefetchHeavyRoutes()).toBe(true)
    expect(shouldPrefetchHeavyRoutes({ effectiveType: '4g' })).toBe(true)
  })

  test('respects data saver, constrained devices and slow networks', () => {
    expect(
      shouldPrefetchHeavyRoutes({ saveData: true, effectiveType: '4g' })
    ).toBe(false)
    ...
    expect(shouldPrefetchHeavyRoutes({ effectiveType: '3g' })).toBe(false)
    expect(shouldPrefetchHeavyRoutes(undefined, 2)).toBe(false)
  })
})
